...
class BrainLMTrainer:
    ...
    def __init__(
        ...
        # OneCycleLR is unstable for very small total_steps and can raise ZeroDivisionError
        # inside its schedule computation. This can happen for tiny per-cluster datasets
        # (few sequences -> few batches), especially during SEQ_LEN/STRIDE sweeps.
        total_steps = int(max_epochs) * int(len(train_loader))
        if total_steps < 10:
            # Keep LR constant; still allows training to proceed.
...
def evaluate_loss(
    ...
    """
    Average masked reconstruction loss on a dataloader (e.g. held-out test).

    Use the same mask_ratio as validation for comparable loss numbers.
    """
    model.eval()
    model = model.to(device)
    total_loss = 0.0
