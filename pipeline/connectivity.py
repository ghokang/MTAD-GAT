...
def compute_connectivity_matrix(
    ...
    Args:
        ...
        use_fisher_z: If True, apply Fisher's r-to-z transform (arctanh) to
            the correlation matrix before returning.
    ...
    if use_fisher_z:
        # Fisher z is undefined at |r|=1, so we clip slightly.
        cm = fisher_z_transform(cm)
...
def compute_all_cms(
    ...
    Args:
        ...
        use_fisher_z: If True, return Fisher z-transformed CMs instead of raw
    ...
    for subject_id, segments in all_segments.items():
        ...
        for segment in segments:
            if segment.length >= min_segment_len:
                cm = compute_connectivity_matrix(segment, method=method, use_fisher_z=use_fisher_z)
                cms.append(cm)
